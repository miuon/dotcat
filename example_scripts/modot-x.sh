#!/bin/bash

urxvtExt=$HOME/.urxvt/ext

print_usage() {
	printf "Usage: no arguments\nShould be run from MODOT
Required environment variables:
\tMODOT_COMMON: directory of common files for this module
\tMODOT_HOST: directory of host files for this module
\tMODOT_THEME: directory of built theme files for this module
Optional environment variables:
\tMODOT_QUICK: true -- quick reload, false -- full restart\n"
}

if [ $# -gt 1 ] || [ -z $MODOT_COMMON ] || \
	[ -z $MODOT_HOST ] || [ -z $MODOT_THEME ] ; then
	print_usage ; exit 1 ;
fi

commonDir=`realpath $MODOT_COMMON`
hostDir=`realpath $MODOT_HOST`
themeDir=`realpath $MODOT_THEME`
isQuick=$MODOT_QUICK

mkdir -p $urxvtExt >/dev/null
ln -sf $commonDir/.xinitrc $HOME/.xinitrc
ln -sf $commonDir/.xserverrc $HOME/.xserverrc
ln -sf $commonDir/.xprofile $HOME/.xprofile
ln -sf $commonDir/.Xresources $HOME/.Xresources
ln -sf $commonDir/config-print $urxvtExt/config-print
ln -sf $commonDir/config-reload $urxvtExt/config-reload
ln -sf $hostDir/.xhost $HOME/.xhost
ln -sf $themeDir/.xcolors $HOME/.xcolors

if [ "$isQuick" = true ]; then
	echo "quick reload x"
	xrdb $HOME/.Xresources
	pkill --signal 1 urxvt
else
	echo "full reload x"
	xrdb $HOME/.Xresources
	pkill --signal 1 urxvt
fi
