...
from dotcat.constants import (MODULE_CONFIG_FILE, ACTIVE_THEME_PATH,
                              ACTIVE_COLOR_PATH)
from dotcat.utils import link_atomic
...
class ThemeEngine():
    ...
    def __init__(self, host_path: Path):
        if not host_path or not host_path.is_file():
            sys.exit('No deployed theme; run dotcat deploy <host_config>')
